...
public class Game extends JFrame implements KeyListener, Runnable {
	...
	private File audioFire; // 총알 사운드
	private File audioHit; // 폭발 사운드

	private int x, y = 450; // 비행기의 좌표
	...
	public Game() {
		...
		setSize(300, 500); // 화면 사이즈 설정
		setVisible(true);
		x = getWidth() / 2 - 15;
		mx = getWidth() / 2 - 2;
		offImage = createImage(getWidth(), getHeight());
		bg = offImage.getGraphics();
		imgBackground = Toolkit.getDefaultToolkit().getImage(getClass().getResource("space.png")); // 배경이미지
		imgGunship = Toolkit.getDefaultToolkit().getImage(getClass().getResource("gunship.png")); // 비행기
		imgMissile = Toolkit.getDefaultToolkit().getImage(getClass().getResource("missile.png")); // 총알
		imgEnemy = Toolkit.getDefaultToolkit().getImage(getClass().getResource("enemy.png")); // 적비행기
		imgHit = Toolkit.getDefaultToolkit().getImage(getClass().getResource("hit.png")); // 폭발이미지
	...
	public void paint(Graphics g) {
		super.paint(g);
		bg.clearRect(0, 0, getWidth(), getHeight()); // 사각형 지우기
		bg.drawImage(imgBackground, 0, 0, this); // 배경이미지
		bg.drawImage(imgGunship, x, y, this); // 비행기
		for (int i = 0; i < mlist.size(); i++) {
			Missile m = mlist.get(i); // 리스트.get(인덱스)
		...
		bg.drawImage(offImage, 0, 0, this);
	}
	...
	public void run() {
		while (true) {
			for (int i = 0; i < mlist.size(); i++) {
				...
				Rectangle rect2 = new Rectangle(m.getMx(), m.getMy(), imgMissile.getWidth(null),
						imgMissile.getHeight(null));
