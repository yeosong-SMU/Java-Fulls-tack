...
public class StudentDAO {
	public Vector listLecture(int studno) {
		...
		try {
			conn = DB.oraConn();
			String sql = "select l.studno, sname, subject_name, pname, point, l.grade from lecture l, stud st, subject sb, prof p where l.studno=st.studno and l.subject_code=sb.subject_code and sb.profno=p.profno and l.studno=?";
	...
	public StudentDTO detailStudent(int studno) {
		...
		try {
			conn = DB.oraConn();
			String sql = "select studno, sname, s.majorno, mname, pname, tel, img_path, p.profno from stud s, prof p, major m where s.profno=p.profno(+) and s.majorno=m.majorno and studno=?";
